...
func (qq *QQ) recognize(msg gjson.Result, cmd command.Command) {
	...
	if isTimeout {
		qq.SendMessage(cq.GetCQResp(cq.ACTION_SEND_MESSAGE, cq.GetMessageQuick(msg, msg.Get(cq.USER_ID).String()+"的操作超时")))
		return
	}
	if cq.IsImageCQCode(result.Get(cq.RAW_MESSAGE).String()) {
		...
		qq.SendMessage(cq.GetCQResp(cq.ACTION_SEND_MESSAGE, cq.GetMessageQuick(msg, saucenao.GetQQSauceNaoResult(result))))
	} else {
